...
Use this the first time a user activates the coach. Ask questions in order. After all answers are collected, write the result to `memory/character_main.md` using the template at the bottom of this file.

## Questions (ask one at a time, skip if user volunteers)

1. **Season** — which Diablo IV season are you playing? (number + name if known)
2. **Class** — Barbarian, Rogue, Sorcerer, Druid, Necromancer, or Spiritborn?
